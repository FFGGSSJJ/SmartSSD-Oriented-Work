...
flow_hairpin(int nb_queues)
{
	...
	for (port_id = 0; port_id < nb_ports; port_id++) {
		pipe = create_hairpin_pipe(ports[port_id], port_id, &error);
		if (pipe == NULL) {
			DOCA_LOG_ERR("Failed to create pipe - %s (%u)\n", error.message, error.type);
			destroy_doca_flow_ports(port_id + 1, ports);
			doca_flow_destroy();
			return -1;
		}

		entry = add_hairpin_pipe_entry(pipe, ports[port_id], &error);
		if (entry == NULL) {
			DOCA_LOG_ERR("Failed to add entry - %s (%u)\n", error.message, error.type);
			destroy_doca_flow_ports(port_id + 1, ports);
			doca_flow_destroy();
			return -1;
		}
